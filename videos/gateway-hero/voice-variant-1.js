// Variant 1: flat flowing color bands. Shared control styling lives in voice.css.
export default function createFlatWaveform(canvas, voiceEnergy, smoothVoiceEnergy) {
    ...
    if (!failed) {
      ...
      for (let row = 0; row < resolution; row += 1) {
        ...
        for (let column = 0; column < resolution; column += 1) {
          ...
          const color = colors[Math.max(0, Math.min(3, Math.floor((0.5 + flow * 0.4) * 4)))];
          pixels.data[pixel++] = color[0];
          pixels.data[pixel++] = color[1];
          pixels.data[pixel++] = color[2];
